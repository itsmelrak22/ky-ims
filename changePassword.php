...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($npass !== $rpass) {
        echo "<script>alert('Error: Passwords do not match!'); window.location='setting.html';</script>";
        exit(); 
    }
    ...
    if ($checkUsernameResult->num_rows != 1) {
        echo "<script>alert('Error: Username doesn't exist!'); window.location='setting.html';</script>";
    ...
    if ($checkEmailResult->num_rows != 1) {
        echo "<script>alert('Error: Email doesn't exist!'); window.location='setting.html';</script>";
    ...
    if ($updateStmt->execute()) {
        echo "<script>alert('Change password successful!'); window.location='setting.html';</script>";
        exit(); // Ensure that no other code is executed after the redirect
    } else {
        echo "<script>alert('Changing password failed. Please try again later.'); window.location='setting.html';</script>";
    }
